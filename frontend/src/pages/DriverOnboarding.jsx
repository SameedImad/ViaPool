      ...
      {STEPS.map((s, i) => (
        <div className="step-item" key={s.n}>
          <div className="step-wrap">
            ...
            <div className={`step-label ${current === s.n ? "active" : current > s.n ? "done" : ""}`}>{s.label}</div>
          </div>
...
function UploadZone({ label, hint, uploaded, onUpload }) {
  const handleDrop = (e) => {
    e.preventDefault();
    onUpload(e.dataTransfer.files[0]?.name || "file");
  };
  return (
    <div
    ...
    >
      ...
      <div className="fz-text">
        {uploaded
          ? <><strong style={{ color: "var(--forest)" }}>{uploaded}</strong><br /><span>Uploaded successfully</span></>
          : <><strong>{label}</strong><br />{hint}</>
        }
      </div>
    </div>
  );
...
export default function DriverOnboarding() {
  ...
  const next = async () => {
    ...
    try {
      ...
      try {
        ...
        localStorage.setItem("appUser", JSON.stringify({ ...currentUser, role: "driver" }));
      } catch (e) {}
      setStep(4);
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)", padding: "40px 24px" }}>
      <a href="/" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none", marginBottom: 40, justifyContent: "center", fontFamily: "var(--font-serif)", fontSize: "1.3rem", color: "var(--ink)" }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        ...
        {step === 1 && (
          <div className="onboard-card">
            <div className="onboard-card-title">Personal <em>details</em></div>
            ...
            <div className="auth-form" style={{ gap: 16 }}>
              <div className="auth-field">
                ...
                <input type="date" className="auth-input" value={personal.dob} onChange={e => setPersonal(p => ({ ...p, dob: e.target.value }))} />
              </div>
              <div className="auth-field">
                <label className="auth-label">Gender</label>
                <select className="auth-input" value={personal.gender} onChange={e => setPersonal(p => ({ ...p, gender: e.target.value }))} style={{ cursor: "pointer" }}>
        ...
        {step === 2 && (
          <div className="onboard-card">
            ...
            <div className="auth-field" style={{ marginTop: 8 }}>
              ...
              <input type="date" className="auth-input" value={license.expiry} onChange={e => setLicense(p => ({ ...p, expiry: e.target.value }))} />
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="onboard-card">
            <div className="onboard-card-title">Your <em>vehicle</em></div>
            ...
            <div className="auth-form" style={{ gap: 14 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div className="auth-field">
                  ...
                  <input className="auth-input" placeholder="Honda" value={vehicle.make} onChange={e => setVehicle(p => ({ ...p, make: e.target.value }))} />
                </div>
                <div className="auth-field">
                  <label className="auth-label">Model</label>
                  <input className="auth-input" placeholder="City" value={vehicle.model} onChange={e => setVehicle(p => ({ ...p, model: e.target.value }))} />
                </div>
                <div className="auth-field">
                  <label className="auth-label">Year</label>
                  <input className="auth-input" placeholder="2022" value={vehicle.year} onChange={e => setVehicle(p => ({ ...p, year: e.target.value }))} />
                </div>
                <div className="auth-field">
                  <label className="auth-label">Type</label>
                  <select className="auth-input" value={vehicle.type} onChange={e => setVehicle(p => ({ ...p, type: e.target.value }))} style={{ cursor: "pointer" }}>
        ...
        {step === 4 && (
          <div className="onboard-card" style={{ textAlign: "center" }}>
            ...
            <div className="onboard-card-title">You're <em>all set!</em></div>
            <p className="onboard-card-sub">Your documents are submitted for review (24–48 hrs). You can explore the dashboard while we verify.</p>
        ...
        {step < 4 && (
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24, maxWidth: 560, margin: "24px auto 0" }}>
            {step > 1
            ...
            >
              {loading ? <><span className="auth-spinner" />Submitting…</> : step === 3 ? "Submit →" : "Continue →"}
            </button>
          </div>
        )}
      </div>
