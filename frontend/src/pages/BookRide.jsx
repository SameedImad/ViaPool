...
const geocodeAddress = async (address) => {
  ...
  if (!bestMatch?.location) {
    throw new Error(`Could not locate "${address}". Please provide a clearer pickup/drop landmark.`);
  }
    ...
    const fetchRide = async () => {
      try {
        ...
        setRide({
          ...
          driver: `${r.driver?.firstName || ""} ${r.driver?.lastName || ""}`.trim() || "Unknown",
          car: `${r.vehicle?.brand || ""} ${r.vehicle?.model || ""}`.trim() || "Vehicle not assigned",
  ...
  const handleConfirm = async () => {
    try {
      const [pickupPoint, dropPoint] = await Promise.all([
        ...
        dropoff ? geocodeAddress(dropoff) : Promise.resolve(undefined),
      ]);
  ...
  if (!ride) {
    return (
      <AppShell title="Not Found" role="passenger">
        <div style={{ padding: 40, textAlign: "center" }}>Ride unavailable</div>
      </AppShell>
    );
  }
        ...
        <div>
          <div className="book-summary">
            ...
            <div className="book-route-row">
              ...
              >
                <div
                  style={{ width: 10, height: 10, borderRadius: "50%", background: "var(--ember)" }}
                />
              ...
              <div style={{ flex: 1 }}>
                <div className="book-city" style={{ marginBottom: 16 }}>
                  {ride.from}
                </div>
          ...
          <div className="seat-picker" style={{ marginBottom: 16 }}>
            ...
            <div className="sp-controls">
              ...
              <button
                className="sp-btn"
                onClick={() => setSeats((s) => Math.min(ride.maxSeats, s + 1))}
                disabled={seats >= ride.maxSeats}
              >
                <Plus size={14} />
              </button>
            </div>
          ...
          <div className="info-card">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <input
                  className="input"
                  placeholder="e.g. Near Inorbit Mall gate 2"
                  value={pickup}
              ...
              <div>
                ...
                <input
                  className="input"
                  placeholder="e.g. Apollo Pharmacy, Banjara Hills"
                  value={dropoff}
              ...
              <div>
                ...
                <textarea
                  className="input"
                  rows={3}
                  placeholder="Any special request or note for the driver..."
                  value={note}
        ...
        <div>
          <div className="info-card" style={{ position: "sticky", top: 88 }}>
            ...
            <div className="fare-row">
              <span>
                Rs.{ride.pricePerSeat} x {seats} seat{seats !== 1 ? "s" : ""}
              </span>
              <span>Rs.{total}</span>
            </div>
            <div className="fare-row total" style={{ paddingTop: 16, marginTop: 4 }}>
              <span>Total</span>
              <span style={{ fontFamily: "var(--font-serif)", fontSize: "1.4rem", letterSpacing: "-0.02em" }}>
                Rs.{total}
              </span>
            </div>
